        ...
        <div class="max-w-screen-md mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <h1 class="text-2xl font-bold mb-6 font-logo">Create Build Order</h1>
                    <form action="{{ route('builds.store') }}" method="POST" class="space-y-4">
                        @csrf
                        <div>
                        ...
                        <div>
                            ...
                            <select name="matchup[]" id="matchup" class="w-full border rounded px-3 py-2 text-gray-900" multiple required>
                                <option value="PPP" {{ in_array('PPP', old('matchup', [])) ? 'selected' : '' }}>PPP</option>
                                <option value="PPT" {{ in_array('PPT', old('matchup', [])) ? 'selected' : '' }}>PPT</option>
                                <option value="PPZ" {{ in_array('PPZ', old('matchup', [])) ? 'selected' : '' }}>PPZ</option>
                                <option value="PTZ" {{ in_array('PTZ', old('matchup', [])) ? 'selected' : '' }}>PTZ</option>
                                <option value="PP" {{ in_array('PP', old('matchup', [])) ? 'selected' : '' }}>PP</option>
                            ...
                            document.addEventListener('DOMContentLoaded', function() {
                                ...
                                function filterMatchups() {
                                    ...
                                    allowed.forEach(val => {
                                        ...
                                        if (opt) matchupSelect.appendChild(opt.cloneNode(true));
                                    });
                                }
                        ...
                        <div>
                            ...
                            <textarea name="description" id="description" class="w-full border rounded px-3 py-2 text-gray-900">{{ old('description') }}</textarea>
                        ...
                        <div class="flex justify-end gap-2">
                            <a href="{{ route('builds.index') }}" class="px-4 py-2 bg-gray-300 rounded text-gray-900 font-nav">Cancel</a>
                            <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded font-nav">Create</button>
                        </div>
                    </form>
                </div>
            </div>
